...
def search_pioneers(request):
    ...
    resp = {
        ...
        "draft_discovery": draft_discovery.pk if draft_discovery else None
    }
...
def search_discoveries(request):
    ...
    if date_start != -1:
        discoveries = discoveries.filter(date_formation__gt=datetime.fromtimestamp(date_start).date())

    if date_end != -1:
        discoveries = discoveries.filter(date_formation__lt=datetime.fromtimestamp(date_end).date())
